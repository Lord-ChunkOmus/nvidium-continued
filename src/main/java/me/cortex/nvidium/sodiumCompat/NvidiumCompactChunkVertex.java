...
public class NvidiumCompactChunkVertex implements ChunkVertexType {
    ...
    public ChunkVertexEncoder getEncoder() {
        return (ptr, material, vertex, sectionIndex) -> {
            int light = compactLight(vertex[0].light);
            MemoryUtil.memPutInt(ptr + 0, (encodePosition(vertex[0].x) << 0) | (encodePosition(vertex[0].y) << 16));
            MemoryUtil.memPutInt(ptr + 4, (encodePosition(vertex[0].z) << 0) | (encodeDrawParameters(material) << 16) | ((light&0xFF)<<24));
            MemoryUtil.memPutInt(ptr + 8, (encodeColor(vertex[0].color) << 0) | (((light>>8)&0xFF) << 24));
            MemoryUtil.memPutInt(ptr + 12, encodeTexture(vertex[0].u, vertex[0].v));
            return ptr + STRIDE;
        };
    }
